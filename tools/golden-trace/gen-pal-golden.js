// The instrument for the palette write-path divergence: every node of
// the reference chip on every half-step of ONE paced $2007 palette
// write and the 96 half-steps after it, so rung 0's replay of the same
// sequence can name the first node that differs. The sequence is the
// write probe's first paced case (a $2006 pair to $3F11 waited out,
// then $2007 <- $21), after the P1 world's warm-up. Writes
// tools/golden-trace/golden-2c02-pal.txt (gitignored, NC-SA-derived):
// a header, then one line per state. About 35 minutes.
'use strict';
const fs = require('fs');
...
run(`R(2); W(0, 0); W(1, 0); W(6, 0x3f); wait(48); W(6, 0x11); wait(96);`);
const start = run('halfSteps');
run(`dumping = true; W(7, 0x21); wait(96); dumping = false;`);
const states = run('OUT.length');
const maxNode = run('maxNode');
console.log(`dumped ${states} states from half-step ${start}, ${maxNode} nodes`);
const out = path.join(__dirname, 'golden-2c02-pal.txt');
const fd = fs.openSync(out, 'w');
fs.writeSync(fd, `2c02 pal golden: start ${start} states ${states} nodes ${maxNode}\n`);
for (let i = 0; i < states; i++) fs.writeSync(fd, run(`OUT[${i}]`) + '\n');
fs.closeSync(fd);
console.log('read back $3F11..: ' + run('hex(readRow(0x11, 2))') + ' (wrote 21)');
console.log('done in', ((Date.now() - t0) / 60000).toFixed(1), 'min; wrote', out);
