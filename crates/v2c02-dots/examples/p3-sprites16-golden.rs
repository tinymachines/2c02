//! The 8x16 golden: a visible frame of the tall-sprite world off rung 0,
...
//! the world wrote. Writes goldens/p3-sprites16.bin (32 palette bytes,
//! 256 OAM bytes, then the 262 x 341 colour bytes) and a stamp.
//! Run with --release; about a minute.

use v2c02_dots::{capture, read_back_palette_and_oam, tall_sprite_world};

fn main() {
    let mut h = tall_sprite_world();
    ...
    std::fs::write(out.join("p3-sprites16.bin"), &bytes).unwrap();
    std::fs::write(
        out.join("p3-sprites16.stamp.txt"),
        format!(
            "2c02 P3 sprite golden: one visible frame of the sprite world off rung 0\n\
             ...
             world: v2c02_dots::tall_sprite_world (standard world + SPRITE_PALETTE + sprite_oam, mask $1E)\n\
             recorded: 2026-09-03 by examples/p3-sprites-golden.rs at {} half-steps\n",
            h.half_steps
        ),
    )
    .unwrap();
    println!("wrote goldens/p3-sprites16.bin ({} bytes)", bytes.len());
}
