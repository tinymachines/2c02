//! The 8x16 sprite gate: the stepper against rung 0's tall-sprite world
//! golden (the sprite world with $2000's bit 5 set), every visible dot,
//! rendering from the palette RAM and OAM the chip was measured to hold;
//! sprite 0's hit against the chip's. What the tall rule is (the height,
//! the halves from consecutive tiles, the bank from the tile's low bit,
//! the vertical flip across both halves) was authored from the published
//! model and is held here to what the chip drew. SKIPs by name without
//! the table or `goldens/p3-sprites16.bin` (written by
//! `p3-sprites16-golden`); `REQUIRE_GOLDEN_P3=1` insists. `MUTATE=1`
//! renders the world's sprites eight tall instead and must go red.

use nes_bus::{ACTIVE_DOTS, ACTIVE_ROWS, DOTS_PER_LINE, LINES};
use v2c02_dots::{tall_sprite_program, standard_program, vram};
use v2c02_fast::{table, Fast};
...
fn golden() -> Option<Golden> {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../goldens/p3-sprites16.bin");
    ...
    match std::fs::read(path) {
        ...
        _ => {
            if std::env::var("REQUIRE_GOLDEN_P3").is_ok() {
                panic!("REQUIRE_GOLDEN_P3 set but goldens/p3-sprites16.bin is missing");
            }
            None
        }
    }
...
fn the_stepper_with_tall_sprites_matches_rung_0_on_every_visible_dot() {
    ...
    for (reg, val, _) in tall_sprite_program() {
        f.write(reg, val);
    }
    ...
    assert_eq!((f.mask, f.ctrl, f.t, f.fine_x), (0x1e, 0x20, 0, 0), "the register file after the tall sprite program");
    ...
    eprintln!(
        "{} visible dots agree with rung 0 with 8x16 sprites on; sprite 0 hit at {:?} (chip {:?}), overflow {:?}",
